...
- **NR-DC band locking.** The Band Locking page's SA NR5G card now has a swap toggle in its header (two arrows). Tap it to flip the slot between SA NR5G and NR-DC, lock the bands you want, and tap it again to switch back — no extra page, no extra card. NR-DC and SA NR5G are locked independently; both are preserved when you switch between them.
...
- **Config backups now include your NR-DC band lock.** Backups already captured your LTE, NSA, and SA band locks; NR-DC is now saved and restored alongside them, so a saved configuration brings back your full band setup. Older backups made before this change restore fine — they simply leave NR-DC untouched.
