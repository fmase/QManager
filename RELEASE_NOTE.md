...
- **Custom SIM Profiles has been redesigned into a single page with a full-width active-profile banner, a clean two-column saved-profiles grid, and a guided multi-step editor that opens in place without leaving the page.** Creating or editing a profile happens entirely in a dialog — five tabs (Identity, APN, Advanced, Scenario, Review) navigable in any order, with a live Review summary and a discard-changes guard if you try to close with unsaved work. Available in all supported languages.
