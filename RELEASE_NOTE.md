...
- **Deactivating a Custom SIM Profile now returns the connection scenario to Balanced.** Previously, deactivating a profile left the modem locked to that profile's network mode (for example, staying on LTE-only when your baseline is Auto). The modem now returns to Auto mode on deactivation, regardless of what the profile had set.
