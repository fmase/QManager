...
- **Verizon support in Custom Profiles.** Selecting **Verizon** as the carrier in Custom SIM Profiles now configures the modem's data routing automatically so both Data and SMS work on Verizon SIMs out of the box. A clear warning dialog explains the special routing rule before you save, and the Profile Slot (CID) is locked to the Verizon-required value while the carrier is selected. **IP Passthrough is automatically locked** while a Verizon profile is active — deactivate it from Custom Profiles to change Passthrough settings. Switching to a non-Verizon profile (or losing your Verizon SIM) cleanly restores default routing and surfaces a reboot reminder. Note: Verizon profiles require USB mode **ECM** or **RNDIS** — the app blocks activation with a clear message if you're on a different mode.
