...
- **Tower locks survive reboots when Signal Failover is on.** Previously, if you locked an LTE or NR-SA cell with Signal Failover enabled and rebooted, the failover watcher could mistake "the poller hasn't published a signal reading yet" for "catastrophic signal loss" and clear the lock during the first 60-90 seconds of boot — even with Persist on at the modem level. The watcher now waits for a confirmed signal value from the poller before it starts scoring samples, so the lock you applied is the lock you have after a reboot.
