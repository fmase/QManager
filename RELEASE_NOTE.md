...
- **Custom SIM Profiles has been rebuilt around a cleaner, single-page editor.** Creating or editing a profile now happens on one focused page that reads top-to-bottom — identity, APN, and an Advanced section (collapsed by default, showing only values that differ from the defaults) followed by the Connection Scenario card. A live summary preview floats to the right on wide screens and stacks below the form on narrow ones. Your saved profiles now appear as a card grid with the active one promoted to the top of the page, and the "Applying profile" screen is a clearer step-by-step status view that shows at a glance what changed and what was left untouched. Available in all supported languages.
