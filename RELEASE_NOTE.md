...
- **Dashboard correctly identifies 5G-SA after a modem reset.** On 5G-SA modems, briefly after an `AT+CFUN=1,1` reset (for example, after applying `qmbncfg="autosel"`), the modem's serving-cell query can still report an LTE anchor while carrier-aggregation reporting already shows NR5G as the primary carrier. The dashboard previously took the stale LTE answer at face value, leaving the **4G Primary Status** card empty and silently dropping the NR primary carrier from the **Secondary Carriers** list. The poller now reconciles the network type from the carrier-aggregation data when the two disagree, so the NR primary and its SCCs render correctly within the next poll cycle — no waiting for the modem to catch up.
