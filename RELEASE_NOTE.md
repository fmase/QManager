...
- **Custom SIM Profiles now carry a Connection Scenario — and an optional daily schedule that switches scenarios automatically.** Each profile can be bound to a scenario (Balanced, Gaming, Streaming, or a custom one) that activates the moment you switch to that profile. Enable the schedule to go further: add up to three daily time windows and QManager will swap scenarios automatically at the boundaries — e.g. Gaming from 8 PM to 6 AM, Balanced the rest of the day. The device handles transitions entirely on its own, even if the app is closed. While a schedule is running, manual scenario changes are blocked with a clear "Scheduled" indicator showing the next switch time.
...
- **Custom SIM Profiles is now a single consolidated page with a step-tabbed profile editor.** Creating or editing a profile happens in place — no separate route, no page navigation. The editor organises fields into five tabs (Identity, APN, Advanced, Scenario, Review) that you can move between in any order, any time. A live preview card pins to the right on wide screens and is available as the Review tab on narrow ones. If you try to leave with unsaved changes, the editor asks before discarding them. Your saved profiles appear as a card grid with the active one promoted to the top, and the "Applying profile" screen shows a step-by-step status view of what changed and what was left untouched. Available in all supported languages.
- **The scenario scheduler in SIM Profile editing is simpler and easier to use.** Add up to three daily time windows — each one just a start time, an end time, and the scenario to run. Every window repeats every day, so there are no day-of-week checkboxes to juggle. Each entry collapses to a one-line summary (`22:00–06:00 → Gaming`) so you can scan your whole schedule at a glance. Reorder with the up/down arrows when windows overlap — the topmost one wins. While the schedule is on, a live line shows which scenario is active right now and when the next switch happens. An "Ends next day" note appears under the End time when a window crosses midnight.
