...
- **Redesigned APN Management — simpler, accurate, focused on your data connection.** The APN editor now shows only the profiles that carry your internet traffic; IMS (VoLTE) and emergency (SOS) contexts provisioned by your carrier are hidden so they can't be accidentally changed. The "In Use" indicator is now anchored on the actual WAN-bearing connection reported by the modem, fixing a bug where the IMS context could incorrectly show as connected. A "For Internet" hint in the CID picker marks the slot your carrier uses for data, making it clear which profile to edit. Removed fields that only worked on Casa firmware (auth type, MTU) — what remains works correctly on all supported OpenWRT modems. Available in all supported languages.
...
- **Custom SIM Profiles is now fully wired and functional.** Create, edit, activate, deactivate, and delete profiles from a clean two-column page — the left side is the form (four tabs: Identity, Network, Scenario, and a live Review summary), and the right side is your saved profiles list. Carrier presets auto-fill APN, TTL, and Hop Limit for common operators. "Load from SIM" reads the inserted SIM's ICCID, IMEI, and active APN into the form with one click. Activation runs in the background and confirms with a notification when done.
