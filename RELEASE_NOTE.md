...
- **Incoming SMS messages that were silently stored on your SIM card now appear in the inbox.** On some carriers and SIM configurations, the modem routes incoming texts to the SIM (35-slot storage) rather than modem memory (255-slot storage) — messages would accumulate there invisibly until the SIM was nearly full. QManager now reads from both locations, merges them into a single inbox, and marks SIM-stored messages with a "SIM" badge so you can tell at a glance which memory they came from. Deleting a SIM message (individually or via Clear All) removes it from both the inbox and the SIM. The correct incoming-storage routing is also set at boot so future messages go to modem memory automatically.
