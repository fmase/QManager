...
The dominant visual reference is **Ubiquiti UniFi** (Network Controller, UniFi OS, Protect). QManager adopts UniFi's dense pill-driven data tables, its varied-size widget mosaic dashboard composition (never uniform card grids), its topology-map approach to network visualization (applied here to cell-tower constellations, antenna geometry, MIMO stream layouts), and its live-updating tiles that tick smoothly without flashing. **Nokia FastMile 5G Gateway 7** contributes the big circular signal-quality meter as a hero readout, and the friendly-but-technical card treatment on signal/antenna pages. **Askey's iF Design Award-winning CPE Management Utility** lends the editorial whitespace and bolder typographic hierarchy on dashboard heroes — the aspirational reminder that a CPE interface can win design awards, not just function correctly.
...
- **Dashboards compose as varied-size mosaics**, never uniform grids. Hero widgets (topology, signal meter, throughput chart) flank smaller stat tiles in deliberate asymmetric layout.
- **Topology and network visualizations are first-class UI surfaces** — cell tower constellations, antenna geometry, neighbor-cell relationships rendered visually, not buried in tables.
...
**The Mosaic Rule (UniFi heritage).** Dashboards compose as varied-size widget mosaics, never uniform card grids. Every dashboard surface must include at least one *hero widget* (topology map, circular signal meter, throughput chart, or large numeric card spanning multiple columns) flanked by smaller stat tiles. A page of identical-size cards is a failure of composition. If you find yourself reaching for `grid-cols-3` with five identical card children, stop and pick a hero.
...
- **Do** compose dashboards as varied-size mosaics with at least one hero widget per surface (per the Mosaic Rule). Never an all-identical card grid.
...
- **Do** leave generous editorial whitespace around dashboard hero sections (Askey iF-award influence). Crowding the hero is what cheapens it.
...
- **Don't** ship the hero-metric SaaS template (big-number-with-gradient-accent-plus-three-supporting-stats). The Signal Dashboard numeric cards are the *anti-template*: tabular, color-coded by quality, no decoration.
- **Don't** ship identical card grids (same-sized icon-plus-heading-plus-text card, repeated). Violates the Mosaic Rule. UniFi never does this; neither do we. If you need four similar things, use a table, a tighter list, or a mosaic with one hero and three smaller tiles.
