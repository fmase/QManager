...
- **`last_iccid` acknowledgement on activation**: `qmanager_profile_apply` calls `mark_sim_acknowledged()` (defined in `profile_mgr.sh`) at each `set_active_profile` success site — line ~401 (pre-CFUN IMEI path) and line ~543 (FINALIZE complete/partial branch). The helper issues `AT+QCCID`, parses the response with `grep '+QCCID:' | sed 's/+QCCID: //g' | tr -d '\r '`, and writes the result with `printf '%s'` (no trailing newline) to `/etc/qmanager/last_iccid`. This makes activation a fourth writer of that file alongside `qmanager_poller` (boot, line ~425/456) and `qmanager_watchcat` (SIM-failover site ~639 and SIM-revert site ~435). **Invariant**: the helper's QCCID parse pipeline and `printf '%s'` write MUST stay byte-identical to the poller's detector — a format divergence causes the boot-time compare (`stored != current`) to false-fire the "New SIM card detected" banner. The helper reads a fresh QCCID from the modem (not the profile's stored `sim_iccid`, which may be hand-typed or differently formatted) and skips silently on an empty read — never clobbers. Called only on activation success; never on deactivate or failure. No reboot path added.
