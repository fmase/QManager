...
| APN Management | `cellular/apn.sh` (actions: save/activate/clear); shared lib: `apn_mgr.sh` (v2 config I/O + COPS apply + `reapply_active_apn_slot`) | `use-wan-profiles.ts` (`saveProfile`, `activateProfile`, `clearProfile`) | `wan-profiles.ts` (`WanProfile`, `CidContext`, `WanProfilesResponse`) | No |
...
| [APN Management](apn-management.md) | 5-slot radio-select model, save vs. activate split, COPS detach/attach cycle, IMS/SOS CID tagging + AlertDialog confirm, v2 config migration, Custom SIM Profile override gate |
