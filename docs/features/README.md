...
| Call Forwarding | `cellular/call_forwarding.sh` (GET; POST: `set{number}`, `disable`) | `use-call-forwarding.ts` (`CallForwardingStatus`, `CallForwardingState`) | — (inline in hook) | No |
...
| [SMS Forwarding & Call Forwarding](sms-call-forwarding.md) | SMS forwarding daemon (`qmanager_sms_forward`): seed-on-first-run, loop guard, 3-attempt abandon + failure file (capped 20), djb2 fingerprint internal-only (UTF-8 awk ≠ UTF-16 frontend). Call forwarding (AT+CCFC unconditional): `cf_network_rejected` CME-257 is a first-class state checked before any `+CCFC:` parse, `grep +CCFC:` anchor skips echo line, `last_number` UCI is prefill-only |
